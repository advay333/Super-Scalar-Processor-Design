...
package branch_rs_types is
    ...
    type branch_rs_entry_t is record
        ...
        branch_tag     : std_logic_vector(B_TAG_WIDTH-1 downto 0);
        pred_dir       : std_logic; 
    ...
    constant EMPTY_BRANCH_ENTRY : branch_rs_entry_t := (
        busy => '0', ip_addr => x"0000", 
        op1_data => x"0000", op1_tag => "000000", v1 => '0',
        op2_data => x"0000", op2_tag => "000000", v2 => '0',
        imm => x"0000", rrf_dest => "000000", control => (others => '0'),
        branch_tag => (others => '0'), pred_dir => '0',
...
entity branch_reservation_station is
    generic (
        RS_DEPTH : integer := 4 
    );
    port (
        ...
        data_in_0  : in  branch_rs_entry_t;
        enq_en_1   : in  std_logic;
        data_in_1  : in  branch_rs_entry_t;

        deq_en     : in  std_logic;

      
        top_data   : out branch_rs_entry_t;
...
architecture rtl of branch_reservation_station is

    type rs_ram_t is array (0 to RS_DEPTH-1) of branch_rs_entry_t;
...
begin
    ...
    top_data   <= fifo_ram(head) when count > 0 else EMPTY_BRANCH_ENTRY;
    
    top_ready  <= '1' when (fifo_ram(head).v1 = '1' and 
                            fifo_ram(head).v2 = '1' and 
                            fifo_ram(head).zero_valid = '1' and 
                            fifo_ram(head).carry_valid = '1' and 
                            count > 0) else '0';
    ...
    process(clk)
        ...
        variable enq_fwd_0  : branch_rs_entry_t;
        variable enq_fwd_1  : branch_rs_entry_t;
...
--                v_is_enq_1 := (enq_en_1 = '1' and (count + 1) < RS_DEPTH);
                ...
                for i in 0 to RS_DEPTH-1 loop
                    if fifo_ram(i).busy = '1' then
                        for b in 0 to 3 loop
                            ...
                            --  Check Independent Flags
                            --  Zero Flag
                            if fifo_ram(i).zero_valid = '0' and fifo_ram(i).zero_flag_tag = cdb_buses(b)(38 downto 33) and cdb_buses(b)(25) = '1' then
                                fifo_ram(i).zero_flag <= cdb_buses(b)(26);
                                fifo_ram(i).zero_valid <= '1';
                            end if;
                            
                            --  Carry Flag
                            if fifo_ram(i).carry_valid = '0' and fifo_ram(i).carry_flag_tag = cdb_buses(b)(32 downto 27) and cdb_buses(b)(23) = '1' then
                                fifo_ram(i).carry_flag <= cdb_buses(b)(24);
                                fifo_ram(i).carry_valid <= '1';
                            end if;
                        end loop;
                    end if;
                end loop;
                ...
                if v_is_enq_0 then
                    ...
                    for b in 0 to 3 loop
                        ...
                        if enq_fwd_0.zero_valid = '0' and enq_fwd_0.zero_flag_tag = cdb_buses(b)(38 downto 33) and cdb_buses(b)(25) = '1' then
                            enq_fwd_0.zero_flag := cdb_buses(b)(26);
                            enq_fwd_0.zero_valid := '1';
                        end if;
                        if enq_fwd_0.carry_valid = '0' and enq_fwd_0.carry_flag_tag = cdb_buses(b)(32 downto 27) and cdb_buses(b)(23) = '1' then
                            enq_fwd_0.carry_flag := cdb_buses(b)(24);
                            enq_fwd_0.carry_valid := '1';
                        end if;
                    end loop;
                ...
                if v_is_enq_1 then
                    ...
                    for b in 0 to 3 loop
                        ...
                        -- 0-Cycle Forwarding for Flags
                        if enq_fwd_1.zero_valid = '0' and enq_fwd_1.zero_flag_tag = cdb_buses(b)(38 downto 33) and cdb_buses(b)(25) = '1' then
                            enq_fwd_1.zero_flag := cdb_buses(b)(26);
                            enq_fwd_1.zero_valid := '1';
                        end if;
                        if enq_fwd_1.carry_valid = '0' and enq_fwd_1.carry_flag_tag = cdb_buses(b)(32 downto 27) and cdb_buses(b)(23) = '1' then
                            enq_fwd_1.carry_flag := cdb_buses(b)(24);
                            enq_fwd_1.carry_valid := '1';
                        end if;
                    end loop;
                ...
                if v_is_deq then
                    ...
                    -- Reset flags on dequeue
                    fifo_ram(head).zero_valid  <= '0';
                    fifo_ram(head).carry_valid <= '0';
